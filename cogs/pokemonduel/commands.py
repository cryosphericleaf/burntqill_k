...
class PokemonDuel(commands.Cog):
    ...
    def __init__(self, bot: QillBot):
        self.bot = bot
        self.collection = self.bot.db['splayer']
        self.games = {}
    ...
    async def _start_duel(self, ctx, opponent: discord.Member, *, inverse_battle=False):
        ...
        for player in (ctx.author, opponent):
            party_doc = None
            try:
                party_doc = await self.collection.find_one({'user_id': str(player.id)})
            except Exception as e:
                print(e)
            party = party_doc['party']
    ...
    async def party_set(self, ctx, *, pokemon_data):
        ...
        await self.collection.update_one({'user_id': user_id}, {'$set': party_data}, upsert=True)
